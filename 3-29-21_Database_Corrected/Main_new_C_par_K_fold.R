Main_new_C_par<-function(input, seed, traits, K, obj_best, Iter){
  ...
  for (i in 1:K){
    tr_row_index[[i]]<-1:nrow(DB)
    va_row_index[[i]]<-vector("integer")
  }
  for_test<-sample(1:nrow(DB), K)
  for (i in 1:K){
    current_num<-for_test[i]
    pin_up<-current_num
    pin_down<-current_num
    diff<-0
    GEO<-DB[current_num, "GEO.accession"]
    repli<-as.integer(substr(DB[current_num, "Replicate"], 2, 2))
    while ((as.integer(substr(DB[pin_up, "Replicate"], 2, 2) == repli - diff) & DB[pin_up, "GEO.accession"] == GEO)){
      pin_up<-pin_up - 1
      diff<-diff + 1
    } 
    diff<-0
    while ((as.integer(substr(DB[pin_down, "Replicate"], 2, 2) == repli + diff) & DB[pin_down, "GEO.accession"] == GEO)){
      pin_down<-pin_down + 1
      diff<-diff + 1
    }
    
    for_va<-pin_up:pin_down
    tr_row_index[[i]]<-tr_row_index[[i]][-for_va]
...
#  }
  ...
  for (i in 1:K){
    ...
    filename_report<-paste("run_report_", i, ".txt", sep="")
    filename_result<-paste("run_result_", i, ".txt", sep="")
    ...
    cat(length(GR_va[[i]]), "experiments used for testing", "\n") 
  ...
  TT<-foreach (i = 1:K) %dopar%{
    ...
    cat("i:", i, "has Tinit of: ", SA_Tinit, ". In run: ", i, '\n')     
    ...
    cat("i:", i, "has Tfinal of: ", SA_Tfinal, ". In run:", i, '\n')     
...
#  foreach(i=1:K, .export=c("Validate", "GetpBad", "sourceCpp", "traits", "X", "Y", "tr_row_index", "va_row_index", "result_train", "G_each_K", "E_each_K", "tr_result", "Iter", "obj_best", "seed", "K")) %dopar% {
  re_<-foreach(i=1:K) %dopar% {
        result<-Train_SA_C(G_initial[[i]], E_initial[[i]], X_tr[[i]], Y_tr[[i]], GR_tr[[i]], X_va[[i]], Y_va[[i]], GR_va[[i]], 1, 1, i, TT[[i]], weight[[i]])
	print(length(result))
	print(class(result))
	return(result)
  }
